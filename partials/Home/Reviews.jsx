    ...
    return (
        <section className="reviews bg-lightgrey md:my-36 my-20" id="reviews">
            <div className="container px-6 sm:px-10 md:px-16 py-14 lg:mx-auto relative">
                {
                    activeIndex !== 0 && (
                        <a href="javascript:;" onClick={() => changeIndex(activeIndex - 1)} className="reviews-button reviews-button--left absolute left-0 top-0 bottom-0 m-auto hidden lg:flex w-12 h-12 rounded-full justify-center items-center cursor-pointer bg-white" data-aos="fade-right">
...

                {
                    activeIndex === reviewsContent.length - 1 === false && (
                        <a href="javascript:;" onClick={() => changeIndex(activeIndex + 1)} className="reviews-button reviews-button--right cursor-pointer absolute right-0 top-0 bottom-0 m-auto hidden lg:flex w-12 h-12 rounded-full items-center justify-center bg-white" data-aos="fade-left">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M5.5 12L11.5 6M18 12L5.5 12L18 12ZM5.5 12L11.5 18L5.5 12Z" stroke="#BABABA" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                        </a>
                ...
                <div className="reviews-paginate absolute flex items-center justify-center left-0 right-0 bottom-14 m-auto">
                    {
                        reviewsContent.map((item, index) => {
                            return activeIndex === index ? (
                                <a href="javascript:;" key={index} onClick={() => changeIndex(index)} className="bg-blue w-3 h-3 mx-1 rounded-full flex cursor-pointer"></a>
                            ) : (
                                <a href="javascript:;" key={index} onClick={() => changeIndex(index)} className="bg-blue w-3 h-3 mx-1 rounded-full opacity-60 flex cursor-pointer"></a>
                            )
                        })
                    }
                </div>
